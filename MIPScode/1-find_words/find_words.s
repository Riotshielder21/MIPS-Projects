...
loopmain:	#main deterministic loop

	lb $t6, 0($t5)
	beq $t6, 0x00, main_end
	
	bne $t6, $t2, princhar #print char if not whitespace
	
	li $v0, 4           
	move $a0, $s0	# print_char in register $t2
	syscall

	addi $t5, $t5, 1 # increment the byte
